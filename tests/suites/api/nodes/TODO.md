...
| API-NODES-002 | After the same 200 ops, `depth == path segments - 1` for every node | property | P0 |
...
| API-NODES-006 | Moving a node onto itself is rejected | unit | P0 |
| API-NODES-007 | Exceeding `MAX_DEPTH` returns 400, not a raw Postgres index error | integration | P0 |

### Move and cascade

| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| API-NODES-008 | Moving a subtree rewrites the paths of every descendant | integration | P0 |
| API-NODES-009 | Cascade soft-delete marks the whole subtree in one transaction | integration | P0 |
...
| API-NODES-012 | Names are NFC-normalized before the uniqueness check | integration | P0 |
| API-NODES-013 | A name containing `%` or `_` cannot break a prefix query | security | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| API-NODES-015 | Listing returns folders before files, then by name, then by id | integration | P0 |
| API-NODES-016 | Pagination over 500 children with Cyrillic and accented names loses and duplicates nothing | integration | P0 |
| API-NODES-017 | Cursor collation matches the `ORDER BY` collation exactly | integration | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| API-NODES-019 | Subtree stats match a naive recursive walk | property | P0 |
| API-NODES-020 | `rebuildSubtree` reconstructs identical paths from `parent_id` alone | integration | P0 |
