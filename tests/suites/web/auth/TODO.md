...
| --- | --- | --- | --- |
| WEB-AUTH-014 | A seeded user signs in and lands in the app | unit | P1 |
| WEB-AUTH-015 | An email typed in a different case still signs in | unit | P1 |
| WEB-AUTH-016 | Leading and trailing whitespace in the email is trimmed before submit | unit | P1 |
| WEB-AUTH-017 | An empty email shows a field error and issues no request | unit | P1 |
| WEB-AUTH-018 | An empty password shows a field error and issues no request | unit | P1 |
...
| WEB-AUTH-021 | Two rapid Enter presses submit once | unit | P1 |
| WEB-AUTH-022 | The password input is `type=password` and never rendered as text | security | P1 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-AUTH-026 | A Google sign-in for a seeded user lands in the app | unit | P1 |
| WEB-AUTH-027 | Closing the Google popup shows no error at all | unit | P1 |
| WEB-AUTH-028 | A Google error response renders the same generic message as a bad password | security | P1 |
| WEB-AUTH-029 | An unknown Google account renders that same message and offers no sign-up path | security | P1 |
...
| WEB-AUTH-034 | A failed submit re-enables the form | unit | P1 |

### Landing in the right place

| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-AUTH-035 | A deep link visited while logged out returns to that link after sign-in | unit | P1 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-AUTH-039 | A full page reload keeps the session | unit | P1 |
| WEB-AUTH-040 | An expired access token on first load refreshes silently and renders the app | unit | P1 |
| WEB-AUTH-041 | A failed refresh on first load lands on login with no flash of app content | unit | P1 |
| WEB-AUTH-042 | A refresh token that expires mid-session sends the user to login once, not repeatedly | unit | P1 |
| WEB-AUTH-043 | Tokens cleared in another tab log this tab out | unit | P1 |
| WEB-AUTH-044 | A corrupted token in storage reads as logged out, never as a crash | unit | P1 |
| WEB-AUTH-045 | The browser back button after sign-out shows no cached app content | security | P1 |

### What is deliberately absent

| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-AUTH-001 | The login page renders no register or sign-up link | security | P1 |
| WEB-AUTH-002 | The login page renders no link to an unimplemented flow | unit | P1 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-AUTH-007 | Logout calls the API before clearing local state | security | P1 |
...
| WEB-AUTH-048 | Logout still clears local state when the API call fails | security | P1 |
...
| WEB-AUTH-005 | Bootstrap renders nothing until `/me` settles — no flash of the login screen | unit | P1 |
| WEB-AUTH-006 | After login the user lands on the originally requested URL | unit | P1 |
| WEB-AUTH-012 | `ProtectedRoute` redirects an anonymous user away from an owner route | security | P1 |
| WEB-AUTH-013 | `ProtectedRoute` never gates `/s/:token` | security | P0 |
| WEB-AUTH-049 | An admin-only route is hidden from a non-admin rather than shown and rejected | security | P1 |
