...
| --- | --- | --- | --- |
| WEB-UPLOADS-011 | Dropping one file onto the content area queues it | unit | P0 |
| WEB-UPLOADS-012 | Choosing files through the file picker queues them identically | unit | P0 |
| WEB-UPLOADS-013 | Dropping 20 files queues 20 items | unit | P0 |
...
| WEB-UPLOADS-017 | Files land in the folder currently being viewed | unit | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-UPLOADS-020 | A file over `MAX_FILE_SIZE` is rejected before any request | unit | P0 |
...
| WEB-UPLOADS-023 | A non-PDF is rejected client-side when PDF-only is enforced | unit | P0 |
| WEB-UPLOADS-024 | A mixed drop uploads the valid files and reports only the rejected ones | unit | P0 |
| WEB-UPLOADS-025 | An all-rejected drop leaves the queue untouched | unit | P1 |
| WEB-UPLOADS-026 | A file whose name collides is uploaded and reported under its resolved name | unit | P0 |

### Progress and feedback

| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-UPLOADS-027 | Progress advances from real XHR events, not a simulated timer | unit | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-UPLOADS-032 | Cancelling one file aborts its request and leaves the others running | unit | P0 |
| WEB-UPLOADS-033 | Cancelling calls `/abort` so the pending node is cleaned up | unit | P0 |
...
| WEB-UPLOADS-036 | Retry after a network failure resumes from init, not from a stale URL | unit | P0 |
| WEB-UPLOADS-037 | Retrying does not duplicate the file | unit | P0 |
...
| WEB-UPLOADS-038 | Navigating two folders away keeps every transfer running | unit | P0 |
| WEB-UPLOADS-039 | Closing the tab during a transfer triggers the beforeunload warning | unit | P0 |
| WEB-UPLOADS-040 | No beforeunload warning appears when the queue is idle | unit | P1 |
| WEB-UPLOADS-041 | Losing the network mid-transfer marks the file failed rather than hanging | unit | P0 |
| WEB-UPLOADS-042 | A presigned URL that expires mid-transfer surfaces as a retryable failure | unit | P1 |
| WEB-UPLOADS-043 | An access token expiring mid-transfer does not lose the queue | unit | P0 |

### Queue management

| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-UPLOADS-002 | Concurrency is capped at 3 regardless of how many files are dropped | unit | P0 |
| WEB-UPLOADS-044 | A fourth file starts only when a slot frees | unit | P0 |
| WEB-UPLOADS-045 | Dropping more files while uploading appends rather than restarting | unit | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-UPLOADS-003 | Each file walks queued to initializing to uploading to completing to done | unit | P0 |
| WEB-UPLOADS-004 | Cancel aborts the request and calls `/abort` | unit | P0 |
...
| WEB-UPLOADS-010 | Queue state lives in zustand, not in the query cache | unit | P0 |
