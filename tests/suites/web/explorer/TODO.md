...
| --- | --- | --- | --- |
| WEB-EXPLORER-013 | Clicking a folder row opens that folder | unit | P0 |
| WEB-EXPLORER-014 | Clicking a file row opens the viewer, not the folder view | unit | P0 |
| WEB-EXPLORER-015 | A breadcrumb segment navigates to that ancestor | unit | P0 |
| WEB-EXPLORER-016 | The browser back button returns to the previous folder | unit | P0 |
| WEB-EXPLORER-017 | A folder URL opened directly renders that folder, not the root | unit | P0 |
| WEB-EXPLORER-018 | Reloading inside a nested folder stays there | unit | P0 |
| WEB-EXPLORER-019 | Navigating to a deleted folder renders a gone state, not a spinner | unit | P0 |
| WEB-EXPLORER-020 | Navigating to a folder id that never existed renders not-found | unit | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-EXPLORER-022 | Creating a folder with a valid name adds it to the list without a refetch flash | unit | P0 |
| WEB-EXPLORER-023 | An empty name is rejected before the request | unit | P0 |
| WEB-EXPLORER-024 | A whitespace-only name is rejected before the request | unit | P0 |
| WEB-EXPLORER-025 | A name over `MAX_NAME_LENGTH` is rejected with the limit shown | unit | P0 |
| WEB-EXPLORER-026 | Leading and trailing whitespace is trimmed rather than rejected | unit | P1 |
| WEB-EXPLORER-027 | A name containing a path separator is rejected or sanitised, never sent raw | security | P0 |
...
| WEB-EXPLORER-030 | A duplicate name surfaces the 409 with the server's `suggestedName` | unit | P0 |
| WEB-EXPLORER-031 | Accepting the suggested name creates the folder in one further click | unit | P0 |
| WEB-EXPLORER-032 | Cancelling the dialog creates nothing | unit | P0 |
| WEB-EXPLORER-033 | Escape closes the dialog and creates nothing | unit | P1 |
| WEB-EXPLORER-034 | A failed create rolls the optimistic row back out of the list | unit | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-EXPLORER-036 | Renaming updates the row in place | unit | P0 |
| WEB-EXPLORER-037 | Renaming to the identical name closes without an error | unit | P1 |
| WEB-EXPLORER-038 | Renaming to a sibling's name surfaces the conflict and the suggestion | unit | P0 |
| WEB-EXPLORER-039 | Renaming the folder you are inside updates the breadcrumb | unit | P1 |
| WEB-EXPLORER-040 | A failed rename restores the previous name | unit | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-EXPLORER-043 | Dragging a row onto a folder moves it | unit | P0 |
| WEB-EXPLORER-044 | Dragging a folder onto itself is refused with no request | unit | P0 |
| WEB-EXPLORER-045 | Dragging a folder onto its own descendant is refused | unit | P0 |
...
| WEB-EXPLORER-049 | Moving into a folder that already holds that name surfaces the conflict | unit | P0 |
| WEB-EXPLORER-050 | The move dialog warns when the destination is shared | security | P0 |
| WEB-EXPLORER-051 | The warning names which grant would newly apply | security | P1 |
| WEB-EXPLORER-052 | A failed move leaves the row in its original folder | unit | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-EXPLORER-054 | Deleting a file asks for confirmation first | unit | P0 |
...
| WEB-EXPLORER-057 | Cancelling the confirmation deletes nothing | unit | P0 |
| WEB-EXPLORER-058 | Deleting a folder that is shared warns that access will be revoked | security | P0 |
| WEB-EXPLORER-059 | Deleting the folder you are viewing navigates to its parent | unit | P1 |
| WEB-EXPLORER-060 | A failed delete restores the row | unit | P0 |
...
| WEB-EXPLORER-066 | Bulk delete confirms once with the combined counts | unit | P0 |
| WEB-EXPLORER-067 | A partially failed bulk action reports which items failed | unit | P0 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-EXPLORER-069 | Scrolling to the bottom loads the next keyset page once | unit | P0 |
| WEB-EXPLORER-070 | A folder with 500 children scrolls without duplicate or missing rows | unit | P0 |
| WEB-EXPLORER-071 | Creating an item while paginated does not duplicate a row | unit | P1 |
| WEB-EXPLORER-072 | Non-ASCII names sort consistently with the server's order | unit | P0 |
...
| WEB-EXPLORER-074 | Acting on a row deleted in another tab shows a gone state, not a crash | unit | P0 |
| WEB-EXPLORER-075 | Acting on a row renamed elsewhere refreshes rather than writing stale data | unit | P1 |
| WEB-EXPLORER-076 | A 409 from any action never leaves the UI in a pending state | unit | P0 |
...
| WEB-EXPLORER-077 | `readOnly` renders no row context menu | security | P0 |
| WEB-EXPLORER-078 | `readOnly` ignores drag gestures entirely | security | P0 |
| WEB-EXPLORER-079 | `readOnly` renders no upload dropzone | security | P0 |
| WEB-EXPLORER-080 | Keyboard shortcuts that mutate are inert in `readOnly` | security | P0 |

### Existing declarations

| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-EXPLORER-002 | Folders sort before files | unit | P0 |
| WEB-EXPLORER-003 | Breadcrumbs collapse to an ellipsis past depth 4 | unit | P1 |
| WEB-EXPLORER-004 | Create and rename apply optimistically and roll back on error | unit | P0 |
| WEB-EXPLORER-005 | Move and delete are not optimistic | unit | P1 |
| WEB-EXPLORER-006 | A 409 offers the `suggestedName` as a one-click action | unit | P0 |
| WEB-EXPLORER-007 | Double-clicking create submits once | unit | P0 |
