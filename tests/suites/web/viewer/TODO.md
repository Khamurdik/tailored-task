...
| --- | --- | --- | --- |
| WEB-VIEWER-006 | Opening a file renders the PDF | unit | P0 |
...
| WEB-VIEWER-011 | A file that fails to load renders an error with a retry, not a blank frame | unit | P0 |

### Signed-URL lifetime

| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| WEB-VIEWER-001 | The signed URL is fetched on open and never cached | security | P0 |
| WEB-VIEWER-002 | An expired URL refetches rather than erroring | unit | P0 |
| WEB-VIEWER-012 | A modal left open for five minutes recovers on the next interaction | unit | P0 |
| WEB-VIEWER-013 | The signed URL never appears in a query key or the address bar | security | P0 |
| WEB-VIEWER-014 | Reopening the same file requests a fresh URL | security | P0 |
...
| WEB-VIEWER-004 | The same component renders inside a share view | unit | P0 |
| WEB-VIEWER-005 | Loading and unsupported-type states render | unit | P1 |
| WEB-VIEWER-017 | The viewer offers no delete or rename affordance in a share view | security | P0 |
