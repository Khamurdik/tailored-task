...
| JOURNEY-003 | As an owner I drop 20 files into a folder, navigate two levels away, and every upload still completes and reports its result | journey | P1 |
...
| JOURNEY-008 | As an owner I create a folder, upload into it, rename it, move it, and delete it in one session without a stale screen | journey | P1 |
| JOURNEY-009 | As an owner I upload two files with the same name and both land under distinct names | journey | P1 |
| JOURNEY-010 | As an owner I reload the page mid-session and land back where I was, still signed in | journey | P1 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| JOURNEY-002 | As an owner I log in with a password, log out, and log back in with Google to the same account | journey | P1 |
| JOURNEY-013 | As an owner I open a deep link while signed out, sign in, and land on that exact link | journey | P1 |
| JOURNEY-014 | As an owner my session survives past the access-token lifetime without me noticing | journey | P1 |
| JOURNEY-015 | As an owner I sign out in one tab and the other tab stops showing my data | journey | P1 |
...
| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| JOURNEY-017 | As an invited viewer I sign in and see the shared folder in my rooms | journey | P1 |
| JOURNEY-018 | As an invited viewer I can read and download but every mutating control is absent | journey | P1 |
| JOURNEY-019 | As an invited viewer I cannot reach the owner's other rooms by editing the URL | journey | P1 |
| JOURNEY-020 | As a person invited before I had an account, I am seeded and the grant is waiting when I first sign in | journey | P1 |
| JOURNEY-021 | As an invited viewer my access disappears the moment the owner revokes it | journey | P1 |
...
| JOURNEY-022 | As a visitor I browse the whole shared subtree but cannot see anything above its root | journey | P1 |
| JOURNEY-023 | As a visitor I am never shown a sign-in screen | journey | P1 |
...
| JOURNEY-026 | As a visitor my link stops working the moment the owner deletes the folder | journey | P1 |
| JOURNEY-027 | As a visitor with an expired link I get a clear expired screen and no retry loop | journey | P1 |
| JOURNEY-028 | As a signed-in user opening someone's share link, I get the read-only view and not my own UI | journey | P1 |

### Stranger and anonymous

| ID | Behaviour | Kind | Pri |
| --- | --- | --- | --- |
| JOURNEY-029 | As a signed-in stranger, every id from another user's room returns not-found | journey | P1 |
| JOURNEY-030 | As an anonymous visitor, every owner route sends me to sign-in and leaks nothing first | journey | P1 |
| JOURNEY-031 | As a stranger I cannot tell whether a room id exists from any response | journey | P1 |
| JOURNEY-032 | As anyone, a guessed share token returns the invalid screen | journey | P1 |
...
| JOURNEY-035 | As a non-admin owner, the jobs area does not exist for me | journey | P1 |
